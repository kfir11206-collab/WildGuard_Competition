#!/bin/bash
# Runs probe_load_phases.py inside the wildfire image. Arg1: "mmap" (default) or "nommap".
set -e
ROOT=/home/student/Documents/projects/wildfire_detection
NO_MMAP=0; [ "$1" = "nommap" ] && NO_MMAP=1
docker run --rm --runtime nvidia --network host \
  ...
  -e PYTHONUNBUFFERED=1 -e NO_MMAP=$NO_MMAP \
  ...
  bash -lc '
    ...
    echo "QPATH=$QPATH"
    QPATH="$QPATH" python3 -u mmap_sandbox/resident_spike/probe_load_phases.py
  '
